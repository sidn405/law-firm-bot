...
// MOBILE FIXES - JavaScript for iOS/Android compatibility

// Fix for iOS - Replace :has() selector functionality
function handleMobileToggleVisibility() {
    const isMobile = window.innerWidth <= 768;
    
    if (!isMobile) return;
    
    // Get all chat windows and their toggles
    const containers = document.querySelectorAll('.lawfirm-chatbot-container');
    
    containers.forEach(container => {
        const chatWindow = container.querySelector('.lawfirm-chat-window');
        const toggle = container.querySelector('.lawfirm-chat-button');
...
// Fix for picture backgrounds on iOS
function initMobilePictures() {
    const isIOS = /iPhone|iPad|iPod/.test(navigator.userAgent);
    
    if (isIOS) {
        // Ensure picture backgrounds are loaded
        const pictureBgs = document.querySelectorAll('.picture-bg, [style*="background-image"]');
        pictureBgs.forEach(element => {
            // Force repaint
...
// Re-check toggle visibility when windows open/close
// Use both event delegation and mutation observer for compatibility
document.addEventListener('click', function(e) {
    // Delay to allow animations to complete
    setTimeout(handleMobileToggleVisibility, 100);
});

// Observe class changes on chat windows
if (typeof MutationObserver !== 'undefined') {
    const observer = new MutationObserver(handleMobileToggleVisibility);
    
    document.querySelectorAll('.lawfirm-chat-window').forEach(window => {
...
// Handle resize events (Android keyboard, etc.)
let resizeTimer;
