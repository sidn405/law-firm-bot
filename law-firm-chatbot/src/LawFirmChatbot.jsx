...
const LawFirmChatbot = () => {
  ...
  const sendToBackend = async (userMessage) => {
    try {
      ...
      setApiError('Connection to server failed. Running in offline mode.');
      return null;
    }
  };
  ...
  const handleButtonClick = async (option) => {
    ...
    // Send to backend
    await sendToBackend(option.label);
  ...
  const handleTextSubmit = async () => {
    ...
    setTimeout(() => {
      if (nextStepData) {
        ...
        // End of flow - submit to backend if we have contact info
        submitCaseToBackend();
  ...
  const submitCaseToBackend = async () => {
    try {
      // Try to extract contact info
      const contactInfo = collectedData.pi_contact_info || collectedData.other_contact_info || '';
      ...
      // Create client
      if (email) {
        const clientResponse = await fetch(`${API_BASE}/api/clients`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        if (clientResponse.ok) {
          ...
          // Create case
          const caseType = collectedData.start || 'other';
          await fetch(`${API_BASE}/api/cases`, {
            ...
            body: JSON.stringify({
              client_id: clientId,
              case_type: caseType,
              description: collectedData.pi_injury_details || collectedData.other_intro || '',
              intake_data: collectedData
            })
          });
        }
      }
    ...
    <div className="flex flex-col h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
      ...
      <div className="bg-gradient-to-r from-blue-900 to-indigo-900 text-white p-6 shadow-lg">
        <div className="max-w-4xl mx-auto">
          ...
          {apiError && (
            <div className="mt-2 text-sm bg-yellow-600 bg-opacity-20 px-3 py-1 rounded flex items-center gap-2">
